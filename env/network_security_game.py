...
class NetworkSecurityEnvironment(object):
    ...
    def __init__(self, task_config_file) -> None:
        ...
        self._firewall = (
            {}
        )  # dict of all the allowed connections in the environment. Keys `IP` ,values: `set` of `IP` objects.
        ...
        logger.info(
            f"\tSetting rewards - goal:{self._goal_reward}, detection:{self._detection_reward}, step:{self._step_reward}"
        )

        # Set the default parameters of all actionss
        # if the values of the actions were updated in the configuration file
        (
            components.ActionType.ScanNetwork.default_success_p,
            components.ActionType.ScanNetwork.default_detection_p,
        ) = self.task_config.read_env_action_data("scan_network")
        (
            components.ActionType.FindServices.default_success_p,
            components.ActionType.FindServices.default_detection_p,
        ) = self.task_config.read_env_action_data("find_services")
        (
            components.ActionType.ExploitService.default_success_p,
            components.ActionType.ExploitService.default_detection_p,
        ) = self.task_config.read_env_action_data("exploit_services")
        (
            components.ActionType.FindData.default_success_p,
            components.ActionType.FindData.default_detection_p,
        ) = self.task_config.read_env_action_data("find_data")
        (
            components.ActionType.ExfiltrateData.default_success_p,
            components.ActionType.ExfiltrateData.default_detection_p,
        ) = self.task_config.read_env_action_data("exfiltrate_data")
        ...
        self._randomize_goal_every_episode = (
            self.task_config.get_randomize_goal_every_episode()
        )
        ...
        if not self._randomize_goal_every_episode:
            ...
            logger.info(
                "Episodic randomization disabled, generating static goal_conditions"
            )
    ...
    def get_all_actions(self):
        ...
        for net, ips in self._networks.items():
            ...
            actions.add(
                components.Action(
                    components.ActionType.ScanNetwork, {"target_network": net}
                )
            )
            for ip in ips:
                # ServiceFind
                actions.add(
                    components.Action(
                        components.ActionType.FindServices, {"target_host": ip}
                    )
                )
                # DataFind
                actions.add(
                    components.Action(
                        components.ActionType.FindData, {"target_host": ip}
                    )
                )
                ...
                if src_ip != trg_ip:
                    for data_list in self._data.values():
                        for data in data_list:
                            actions.add(
                                components.Action(
                                    components.ActionType.ExfiltrateData,
                                    {
                                        "target_host": trg_ip,
                                        "data": data,
                                        "source_host": src_ip,
            ...
            for service in services:
                for ip, host in self._ip_to_hostname.items():
                    if host_id == host:
                        actions.add(
                            components.Action(
                                components.ActionType.ExploitService,
                                {"target_host": ip, "target_service": service},
                            )
                        )
    ...
    def _create_starting_state(self) -> components.GameState:
        ...
        for controlled_host in self._attacker_start_position["controlled_hosts"]:
            if isinstance(controlled_host, components.IP):
                ...
                known_hosts = self._attacker_start_position["known_hosts"].union(
                    controlled_hosts
                )
                logger.info(
                    f"\tThe attacker has control of host {str(controlled_host)}."
                )
        ...
        game_state = components.GameState(
            controlled_hosts,
            known_hosts,
            self._attacker_start_position["known_services"],
            self._attacker_start_position["known_data"],
            known_networks,
        )
        return game_state
    ...
    def _generate_win_conditions(self, win_conditions) -> dict:
        ...
        if win_conditions["known_networks"] == "random":
            updated_win_conditions["known_networks"] = {
                random.choice(list(self._networks.keys()))
            }
            logger.info("\tRandomizing networks")
        else:
            updated_win_conditions["known_networks"] = copy.deepcopy(
                win_conditions["known_networks"]
            )
        logger.info(
            f"\tGoal known_networks: {updated_win_conditions['known_networks']}"
        )
        # known_hosts
        if win_conditions["known_hosts"] == "random":
            logger.info("\tRandomizing known_host")
            updated_win_conditions["known_hosts"] = {
                random.choice(list(self._ip_to_hostname.keys()))
            }
        else:
            updated_win_conditions["known_hosts"] = copy.deepcopy(
                win_conditions["known_hosts"]
            )
        ...
        if win_conditions["controlled_hosts"] == "random":
            ...
            updated_win_conditions["controlled_hosts"] = {
                random.choice(list(self._ip_to_hostname.keys()))
            }
        else:
            updated_win_conditions["controlled_hosts"] = copy.deepcopy(
                win_conditions["controlled_hosts"]
            )
        logger.info(
            f"\tGoal controlled_hosts: {updated_win_conditions['controlled_hosts']}"
        )
        ...
        for host, service_list in win_conditions["known_services"].items():
            ...
            if isinstance(service_list, str) and service_list.lower() == "random":
                ...
                updated_win_conditions["known_services"][host] = random.choice(
                    available_services
                )
            else:
                updated_win_conditions["known_services"][host] = copy.deepcopy(
                    win_conditions["known_services"][host]
                )
        logger.info(
            f"\tGoal known_services: {updated_win_conditions['known_services']}"
        )
        ...
        for host, data_set in win_conditions["known_data"].items():
            ...
            if isinstance(data_set, str) and data_set.lower() == "random":
                ...
                updated_win_conditions["known_data"][host] = {
                    random.choice(list(available_data))
                }
                logger.info(f"\tRandomizing known_data in {host}")
            else:
                updated_win_conditions["known_data"][host] = copy.deepcopy(
                    win_conditions["known_data"][host]
                )
    ...
    def _place_defences(self) -> None:
        ...
        match defender_type:
            ...
            case "StochasticWithThreshold":
                ...
                self._defender_thresholds["tw_size"] = (
                    self.task_config.get_defender_tw_size()
                )
    ...
    def _process_cyst_config(self, configuration_objects: list) -> None:
        ...
        def process_node_config(node_obj: NodeConfig) -> None:
            ...
            for interface in node_obj.interfaces:
                ...
                logger.info(
                    f"\t\tAdded network {str(interface.net)} to the list of available nets, with node {node_obj.id}."
                )
            ...
            for service in node_obj.passive_services:
                ...
                self._services[node_obj.id].append(
                    components.Service(
                        service.type, "passive", service.version, service.local
                    )
                )
                # data
                logger.info(
                    f"\t\t\tProcessing data in node '{node_obj.id}':'{service.type}' service"
                )
                try:
                    for data in service.private_data:
                        if node_obj.id not in self._data:
                            self._data[node_obj.id] = set()
                        self._data[node_obj.id].add(
                            components.Data(data.owner, data.description)
                        )
    ...
    def _create_new_network_mapping(self):
        ...
        while not valid_valid_network_mapping:
            try:
                ...
                new_base = netaddr.IPNetwork(
                    fake.ipv4_private(), private_nets_sorted[0].mask
                )
                # store its new mapping
                mapping_nets[private_nets[0]] = components.Network(
                    str(new_base.network), private_nets_sorted[0].mask
                )
                ...
                for i in range(1, len(private_nets_sorted)):
                    ...
                    new_net_addr = (
                        netaddr.IPNetwork(str(mapping_nets[private_nets_sorted[0]])).ip
                        + diff_ip
                    )
                    # evaluate if its still a private network
                    is_private_net_checks.append(new_net_addr.is_private())
                    # store the new mapping
                    mapping_nets[private_nets_sorted[i]] = components.Network(
                        str(new_net_addr), private_nets_sorted[i].mask
                    )
                if (
                    False not in is_private_net_checks
                ):  # verify that ALL new networks are still in the private ranges
                ...
                if counter_iter > 10:
                    logger.error(
                        "Dynamic address failed more than 10 times - stopping."
                    )
                    exit(-1)
        ...
        new_attacker_start = {}
        new_attacker_start["known_networks"] = {
            mapping_nets[net] for net in self._attacker_start_position["known_networks"]
        }
        new_attacker_start["known_hosts"] = {
            mapping_ips[ip] for ip in self._attacker_start_position["known_hosts"]
        }
        new_attacker_start["controlled_hosts"] = {
            mapping_ips[ip] for ip in self._attacker_start_position["controlled_hosts"]
        }
        new_attacker_start["known_services"] = {
            mapping_ips[ip]: service
            for ip, service in self._attacker_start_position["known_services"].items()
        }
        new_attacker_start["known_data"] = {
            mapping_ips[ip]: data
            for ip, data in self._attacker_start_position["known_data"].items()
        }
        self._attacker_start_position = new_attacker_start
        # goal definition
        new_goal = {}
        new_goal["known_networks"] = {
            mapping_nets[net] for net in self._goal_conditions["known_networks"]
        }
        new_goal["known_hosts"] = {
            mapping_ips[ip] for ip in self._goal_conditions["known_hosts"]
        }
        new_goal["controlled_hosts"] = {
            mapping_ips[ip] for ip in self._goal_conditions["controlled_hosts"]
        }
        new_goal["known_services"] = {
            mapping_ips[ip]: service
            for ip, service in self._goal_conditions["known_services"].items()
        }
        new_goal["known_data"] = {
            mapping_ips[ip]: data
            for ip, data in self._goal_conditions["known_data"].items()
        }
    ...
    def _get_services_from_host(self, host_ip: str, controlled_hosts: set) -> set:
        ...
        if host_ip in self._ip_to_hostname:  # is it existing IP?
            if (
                self._ip_to_hostname[host_ip] in self._services
            ):  # does it have any services?
                if host_ip in controlled_hosts:  # Shoul  local services be included ?
                    found_services = {
                        s for s in self._services[self._ip_to_hostname[host_ip]]
                    }
                else:
                    found_services = {
                        s
                        for s in self._services[self._ip_to_hostname[host_ip]]
                        if not s.is_local
                    }
            else:
    ...
    def _get_data_in_host(self, host_ip: str, controlled_hosts: set) -> set:
        ...
        data = set()
        if (
            host_ip in controlled_hosts
        ):  # only return data if the agent controls the host
    ...
    def _firewall_check_one(self, src_ip: components.IP, dst_ip: components.IP) -> bool:
    ...
    def _firewall_check_all(self, controlled_hosts: set, dst_ip: components.IP) -> bool:
        for src_ip in controlled_hosts:
            if self._firewall_check_one(src_ip, dst_ip):
                return True
        return False

    def _execute_action(
        self, current: components.GameState, action: components.Action
        ...
        if action.type == components.ActionType.ScanNetwork:
            ...
            for ip in self._ip_to_hostname.keys():  # check if IP exists
                logger.info(
                    f"\t\tChecking if {ip} in {action.parameters['target_network']}"
                )
                if str(ip) in netaddr.IPNetwork(
                    str(action.parameters["target_network"])
                ):
                    if self._firewall_check_all(current.controlled_hosts, ip):
            ...
            logger.info(
                f"\t\tSearching for services in {action.parameters['target_host']}"
            )
            if self._firewall_check_all(
                current.controlled_hosts, action.parameters["target_host"]
            ):
                found_services = self._get_services_from_host(
                    action.parameters["target_host"], current.controlled_hosts
                )
                logger.info(f"\t\t\tFound {len(found_services)}: {found_services}")
                if len(found_services) > 0:
                    if (
                        action.parameters["target_host"]
                        not in next_known_services.keys()
                    ):
                        next_known_services[action.parameters["target_host"]] = (
                            found_services
                        )
                    else:
                        next_known_services[action.parameters["target_host"]] = (
                            next_known_services[action.parameters["target_host"]].union(
                                found_services
                            )
                        )
                    ...
                    if action.parameters["target_host"] not in next_known_hosts:
                        logger.info(
                            f"\t\tAdding {action.parameters['target_host']} to known_hosts"
                        )
                        next_known_hosts.add(action.parameters["target_host"])
                        next_known_networks = next_known_networks.union(
                            {
                                net
                                for net, values in self._networks.items()
                                if action.parameters["target_host"] in values
                            }
                        )
            ...
            if self._firewall_check_all(
                ...
                new_data = self._get_data_in_host(
                    action.parameters["target_host"], current.controlled_hosts
                )
                ...
                if len(new_data) > 0:
                    if action.parameters["target_host"] not in next_known_data.keys():
                        ...
                        next_known_data[action.parameters["target_host"]] = (
                            next_known_data[action.parameters["target_host"]].union(
                                new_data
                            )
                        )
            ...
            logger.info(
                f"\t\tAttempting to ExploitService in '{action.parameters['target_host']}':'{action.parameters['target_service']}'"
            )
            if self._firewall_check_all(
                current.controlled_hosts, action.parameters["target_host"]
            ):
                if (
                    action.parameters["target_host"] in self._ip_to_hostname
                ):  # is it existing IP?
                    logger.info("\t\t\tValid host")
                    if (
                        self._ip_to_hostname[action.parameters["target_host"]]
                        in self._services
                    ):  # does it have any services?
                        if (
                            action.parameters["target_service"]
                            in self._services[
                                self._ip_to_hostname[action.parameters["target_host"]]
                            ]
                        ):  # does it have the service in question?
                            if (
                                action.parameters["target_host"] in next_known_services
                            ):  # does the agent know about any services this host have?
                                if (
                                    action.parameters["target_service"]
                                    in next_known_services[
                                        action.parameters["target_host"]
                                    ]
                                ):
                                    ...
                                    if (
                                        action.parameters["target_host"]
                                        not in next_controlled_hosts
                                    ):
                                        next_controlled_hosts.add(
                                            action.parameters["target_host"]
                                        )
                                        logger.info("\t\tAdding to controlled_hosts")
                                    new_networks = self._get_networks_from_host(
                                        action.parameters["target_host"]
                                    )
                                    logger.info(
                                        f"\t\t\tFound {len(new_networks)}: {new_networks}"
                                    )
                                    next_known_networks = next_known_networks.union(
                                        new_networks
                                    )
                                    ...
                                    logger.info(
                                        "\t\t\tCan not exploit. Agent does not know about target host selected service"
                                    )
                            else:
                                logger.info(
                                    "\t\t\tCan not exploit. Agent does not know about target host having any service"
                                )
                        else:
                            logger.info(
                                "\t\t\tCan not exploit. Target host does not the service that was attempted."
                            )
                    else:
                        logger.info(
                            "\t\t\tCan not exploit. Target host does not have any services."
                        )
                else:
                    logger.info("\t\t\tCan not exploit. Target host does not exist.")
        elif action.type == components.ActionType.ExfiltrateData:
            logger.info(
                f"\t\tAttempting to Exfiltrate {action.parameters['data']} from {action.parameters['source_host']} to {action.parameters['target_host']}"
            )
            if self._firewall_check_one(
                action.parameters["source_host"], action.parameters["target_host"]
            ):
                # Is the target host controlled?
                if action.parameters["target_host"] in current.controlled_hosts:
                    logger.info(
                        f"\t\t\t {action.parameters['target_host']} is under-control: {current.controlled_hosts}"
                    )
                    # Is the source host controlled?
                    if action.parameters["source_host"] in current.controlled_hosts:
                        logger.info(
                            f"\t\t\t {action.parameters['source_host']} is under-control: {current.controlled_hosts}"
                        )
                        ...
                        if (
                            ...
                            in current.known_data.keys()
                            and action.parameters["data"]
                            in current.known_data[action.parameters["source_host"]]
                        ):
                            # Does the source host have any data?
                            if (
                                self._ip_to_hostname[action.parameters["source_host"]]
                                in self._data.keys()
                            ):
                                ...
                                if (
                                    ...
                                    logger.info(
                                        "\t\t\t Data present in the source_host"
                                    )
                                    if (
                                        action.parameters["target_host"]
                                        not in next_known_data.keys()
                                    ):
                                        next_known_data[
                                            action.parameters["target_host"]
                                        ] = {action.parameters["data"]}
                                    ...
                                    logger.info(
                                        "\t\t\tCan not exfiltrate. Source host does not have this data."
                                    )
                            else:
                                logger.info(
                                    "\t\t\tCan not exfiltrate. Source host does not have any data."
                                )
                        else:
                            logger.info(
                                "\t\t\tCan not exfiltrate. Agent did not find this data yet."
                            )
                    else:
                        logger.info(
                            "\t\t\tCan not exfiltrate. Source host is not controlled."
                        )
                else:
                    logger.info(
                        "\t\t\tCan not exfiltrate. Target host is not controlled."
                    )
        else:
            raise ValueError(f"Unknown Action type or other error: '{action.type}'")

        return components.GameState(
            next_controlled_hosts,
            next_known_hosts,
            next_known_services,
            next_known_data,
            next_known_networks,
    ...
    def is_goal(self, state: components.GameState) -> bool:
        ...
        def goal_dict_satistfied(goal_dict: dict, known_dict: dict) -> bool:
            ...
            if goal_dict.keys() <= known_dict.keys():
                ...
                try:
                    ...
                    matching_keys = [
                        host
                        for host in goal_dict.keys()
                        if goal_dict[host] <= known_dict[host]
                    ]
                    # Check we have the amount of mathing keys as in the goal_dict
                    logger.info(
                        f"\t\tMatching sets: {len(matching_keys)}, required: {len(goal_dict.keys())}"
                    )
        ...
        logger.info(
            f'\tServices needed to win {self._win_conditions["known_services"]}'
        )
        services_goal = goal_dict_satistfied(
            self._win_conditions["known_services"], state.known_services
        )

        # Data
        logger.info("Checking the goal of data")
        logger.info(f'\tData needed to win {self._win_conditions["known_data"]}')
        known_data_goal = goal_dict_satistfied(
            self._win_conditions["known_data"], state.known_data
        )

        logger.info(
            f"\tnets:{networks_goal}, known_hosts:{known_hosts_goal}, controlled_hosts:{controlled_hosts_goal},services:{services_goal}, data:{known_data_goal}"
        )
        goal_reached = (
            networks_goal
            and known_hosts_goal
            and controlled_hosts_goal
            and services_goal
            and known_data_goal
        )
        return goal_reached
    ...
    def _stochastic_detection_with_thresholds(self, action: components.Action) -> bool:
        ...
        if len(self._actions_played) > 1:  # single action is never detected:
            last_n_actions = self._actions_played[
                -self._defender_thresholds["tw_size"] :
            ]
            ...
            match action.type:  # thresholds are based on action type
                case components.ActionType.ScanNetwork:
                    tw_ratio = (
                        last_n_action_types.count(components.ActionType.ScanNetwork)
                        / self._defender_thresholds["tw_size"]
                    )
                    num_consecutive_scans = max(
                        sum(
                            1
                            for item in grouped
                            if item == components.ActionType.ScanNetwork
                        )
                        for _, grouped in itertools.groupby(last_n_action_types)
                    )
                    if (
                        tw_ratio
                        < self._defender_thresholds[components.ActionType.ScanNetwork][
                            "tw_ratio"
                        ]
                        and num_consecutive_scans
                        < self._defender_thresholds[components.ActionType.ScanNetwork][
                            "consecutive_actions"
                        ]
                    ):
                        ...
                        logger.info(
                            f"\t\t Threshold crossed - TW ratio:{tw_ratio}(T={self._defender_thresholds[components.ActionType.ScanNetwork]['tw_ratio']}), #consecutive actions:{num_consecutive_scans} (T={self._defender_thresholds[components.ActionType.ScanNetwork]['consecutive_actions']})"
                        )
                        return self._stochastic_detection(action)
                case components.ActionType.FindServices:
                    tw_ratio = (
                        last_n_action_types.count(components.ActionType.FindServices)
                        / self._defender_thresholds["tw_size"]
                    )
                    num_consecutive_scans = max(
                        sum(
                            1
                            for item in grouped
                            if item == components.ActionType.FindServices
                        )
                        for _, grouped in itertools.groupby(last_n_action_types)
                    )
                    if (
                        tw_ratio
                        < self._defender_thresholds[components.ActionType.FindServices][
                            "tw_ratio"
                        ]
                        and num_consecutive_scans
                        < self._defender_thresholds[components.ActionType.FindServices][
                        ...
                        logger.info(
                            f"\t\t Threshold crossed - TW ratio:{tw_ratio}(T={self._defender_thresholds[components.ActionType.FindServices]['tw_ratio']}), #consecutive actions:{num_consecutive_scans} (T={self._defender_thresholds[components.ActionType.FindServices]['consecutive_actions']})"
                        )
                        return self._stochastic_detection(action)
                case components.ActionType.FindData:
                    tw_ratio = (
                        last_n_action_types.count(components.ActionType.FindData)
                        / self._defender_thresholds["tw_size"]
                    )
                    if (
                        tw_ratio
                        < self._defender_thresholds[components.ActionType.FindData][
                            "tw_ratio"
                        ]
                        and repeated_action_episode
                        < self._defender_thresholds[components.ActionType.FindData][
                            "repeated_actions_episode"
                        ]
                    ):
                        return False
                    else:
                        logger.info(
                            f"\t\t Threshold crossed - TW ratio:{tw_ratio}(T={self._defender_thresholds[components.ActionType.FindData]['tw_ratio']}), #repeated actions:{repeated_action_episode}"
                        )
                        return self._stochastic_detection(action)
                case components.ActionType.ExploitService:
                    tw_ratio = (
                        last_n_action_types.count(components.ActionType.ExploitService)
                        / self._defender_thresholds["tw_size"]
                    )
                    if (
                        tw_ratio
                        < self._defender_thresholds[
                            components.ActionType.ExploitService
                        ]["tw_ratio"]
                        and repeated_action_episode
                        < self._defender_thresholds[
                        ...
                        logger.info(
                            f"\t\t Threshold crossed - TW ratio:{tw_ratio}(T={self._defender_thresholds[components.ActionType.ExploitService]['tw_ratio']}), #repeated actions:{repeated_action_episode}"
                        )
                        return self._stochastic_detection(action)
                case components.ActionType.ExfiltrateData:
                    tw_ratio = (
                        last_n_action_types.count(components.ActionType.ExfiltrateData)
                        / self._defender_thresholds["tw_size"]
                    )
                    num_consecutive_scans = max(
                        sum(
                            1
                            for item in grouped
                            if item == components.ActionType.ExfiltrateData
                        )
                        for _, grouped in itertools.groupby(last_n_action_types)
                    )
                    if (
                        ...
                        logger.info(
                            f"\t\t Threshold crossed - TW ratio:{tw_ratio}(T={self._defender_thresholds[components.ActionType.ExfiltrateData]['tw_ratio']}), #consecutive actions:{num_consecutive_scans} (T={self._defender_thresholds[components.ActionType.ExfiltrateData]['consecutive_actions']})"
                        )
                        return self._stochastic_detection(action)
    ...
    def reset(self) -> components.Observation:
        ...
        if self._episode_replay_buffer is not None:
            store_replay_buffer_in_csv(
                self._episode_replay_buffer, "env/logs/replay_buffer.csv"
            )
            self._episode_replay_buffer = []
        ...
        return components.Observation(
            self._current_state, initial_reward, self._done, info
        )
    ...
    def step(self, action: components.Action) -> components.Observation:
        ...
        if not self._done:
            ...
            if not is_goal and self._step_counter >= self._max_steps:
                ...
                logger.info(
                    f"Episode ended: Exceeded max number of steps ({self._max_steps})"
                )

            # Save the transition to the episode replay buffer if there is any
            if self._episode_replay_buffer is not None:
                self._episode_replay_buffer.append(
                    (current_state, action, reward, next_state, self._done)
                )
            # Return an observation
            return components.Observation(
                self._current_state, reward, self._done, reason
            )
        else:
            logger.warning(
                "Interaction over! No more steps can be made in the environment"
            )
            raise ValueError(
                "Interaction over! No more steps can be made in the environment"
            )
