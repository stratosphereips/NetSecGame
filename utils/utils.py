...
from env.scenarios import scenario_three_nets
    ...
    try:
        with open(csvfile, "r") as f_object:
            ...
            for [s_t, a_t, r, s_t1, done] in csv_reader:
                buffer.append(
                    (
                        GameState.from_json(s_t),
                        Action.from_json(a_t),
                        r,
                        GameState.from_json(s_t1),
                        done,
                    )
                )
...
def store_replay_buffer_in_csv(
    replay_buffer: list, filename: str, delimiter: str = ";"
) -> None:
    """
    ...
    with open(filename, "a") as f_object:
        ...
        for s_t, a_t, r, s_t1, done in replay_buffer:
            writer_object.writerow(
                [s_t.as_json(), a_t.as_json(), r, s_t1.as_json(), done]
            )
...
def state_as_ordered_string(state: GameState) -> str:
    ...
    for host in sorted(state.known_services.keys()):
        ret += (
            f"{host}:[{','.join([str(x) for x in sorted(state.known_services[host])])}]"
        )
    ret += "},data:{"
...
class ConfigParser:
            ...
            try:
                ...
                for datum in data:
                    if not isinstance(datum, list) and datum.lower() == "random":
                        ...
                        known_data_content = Data(
                            known_data_content_str_user, known_data_content_str_data
                        )
    ...
    def read_agents_known_services(self, type_agent: str, type_data: str) -> dict:
        ...
        """
        known_services_conf = self.config["agents"][type_agent][type_data][
            "known_services"
        ]
        known_services = {}
        for ip, data in known_services_conf.items():
            try:
                ...
                known_services[known_services_host] = Service(
                    name, type, version, is_local
                )
    ...
    def read_agents_known_networks(self, type_agent: str, type_data: str) -> dict:
        ...
        """
        known_networks_conf = self.config["agents"][type_agent][type_data][
            "known_networks"
        ]
        known_networks = set()
    ...
    def read_agents_controlled_hosts(self, type_agent: str, type_data: str) -> dict:
        ...
        """
        controlled_hosts_conf = self.config["agents"][type_agent][type_data][
            "controlled_hosts"
        ]
    ...
    def get_attacker_start_position(self):
        ...
        # Read controlled hosts
        controlled_hosts = self.read_agents_controlled_hosts(
    ...
    def get_defender_thresholds(self):
        ...
        # ScanNetwork
        thresholds[ActionType.ScanNetwork] = {
            "consecutive_actions": config_thresholds["scan_network"][
                "consecutive_actions"
            ]
        }
        thresholds[ActionType.ScanNetwork]["tw_ratio"] = config_thresholds[
            "scan_network"
        ]["tw_ratio"]
        # FindServices
        thresholds[ActionType.FindServices] = {
            "consecutive_actions": config_thresholds["find_services"][
                "consecutive_actions"
            ]
        }
        thresholds[ActionType.FindServices]["tw_ratio"] = config_thresholds[
            "find_services"
        ]["tw_ratio"]
        # FindData
        thresholds[ActionType.FindData] = {
            "repeated_actions_episode": config_thresholds["find_data"][
                "repeated_actions_episode"
            ]
        }
        thresholds[ActionType.FindData]["tw_ratio"] = config_thresholds["find_data"][
            "tw_ratio"
        ]
        # ExploitService
        thresholds[ActionType.ExploitService] = {
            "repeated_actions_episode": config_thresholds["exploit_service"][
                "repeated_actions_episode"
            ]
        }
        thresholds[ActionType.ExploitService]["tw_ratio"] = config_thresholds[
            "exploit_service"
        ]["tw_ratio"]
        # ExfiltrateData
        thresholds[ActionType.ExfiltrateData] = {
            "consecutive_actions": config_thresholds["exfiltrate_data"][
                "consecutive_actions"
            ]
        }
        thresholds[ActionType.ExfiltrateData]["tw_ratio"] = config_thresholds[
            "exfiltrate_data"
        ]["tw_ratio"]
        return thresholds
    ...
    def get_scenario(self):
        ...
        if scenario == "scenario1":
            ...
            cyst_config = scenario_three_nets.configuration_objects
        else:
    ...
    def get_randomize_goal_every_episode(self) -> bool:
        ...
        """
        try:
            randomize_goal_every_episode = self.config["agents"]["attacker"]["goal"][
...
if __name__ == "__main__":
    state = GameState(
        known_networks={Network("1.1.1.1", 24), Network("1.1.1.2", 24)},
        known_hosts={IP("192.168.1.2"), IP("192.168.1.3")},
        controlled_hosts={IP("192.168.1.2")},
        known_services={
            IP("192.168.1.3"): {Service("service1", "public", "1.01", True)}
        },
        known_data={
            IP("192.168.1.3"): {
                Data("ChuckNorris", "data1"),
                Data("ChuckNorris", "data2"),
            },
