...
def state_diff(s1: GameState, s2: GameState) -> float:
    diff = len(s1.known_networks - s2.known_networks) + len(
        s2.known_networks - s1.known_networks
    )
    diff += len(s1.known_hosts - s2.known_hosts) + len(s2.known_hosts - s1.known_hosts)
    diff += len(s1.controlled_hosts - s2.controlled_hosts) + len(
        s2.controlled_hosts - s1.controlled_hosts
    )

    diff_services = 0
    for key in s1.known_services.keys():
        if key in s2.known_services:
            diff_services += len(s1.known_services[key] - s2.known_services[key]) + len(
                s2.known_services[key] - s1.known_services[key]
            )
        else:
    ...
    for key in s1.known_data.keys():
        if key in s2.known_data:
            diff_data += len(s1.known_data[key] - s2.known_data[key]) + len(
                s2.known_data[key] - s1.known_data[key]
            )
        else:
...
def action_diff(a1: Action, a2: Action) -> float:
    ...
    src_host_diff = (
        0 if a1.parameters["source_host"] == a2.parameters["source_host"] else 1
    )
...
def plot_barplot(data: dict, fileneme, title="ActionType distribution per step"):
    ...
    for action_type, values in action_counts.items():
        ax.bar(
            data.keys(),
            values,
            label=str(action_type).lstrip("ActionType."),
            bottom=bottom,
        )
...
def get_action_type_histogram_per_step(
    game_plays: list, end_reason=None, filename="action_type_histogram.png"
):
    """
...
def get_action_type_barplot_per_step(
    game_plays: list, end_reason=None, filename="action_type_barplot.png"
):
    """
...
def barplot_action_efficiency(
    data: list,
    model_names: list,
    filename="action_efficiency_plot.png",
    end_reason=None,
):
    ...
    for i, trajectories in enumerate(data):
        ...
        for action_type in action_data:
            action_data[action_type][i] = np.round(
                num_trajectories / action_data[action_type][i], decimals=2
            )
...
def cluster_trajectory_steps(
    game_plays: list, filename, end_reason=None, y="timestamp"
):
    action_conversion = {
    ...
    if y == "timestamp":
        plt.scatter(
            embedding[:, 0],
            embedding[:, 1],
            c=[x[1] for x in trajectory_steps],
            cmap="plasma",
            s=5,
            alpha=0.5,
        )
        ...
        cm = from_list(
            "Set15",
            plt.cm.tab10(range(0, len(action_conversion))),
            len(action_conversion),
        )
        plt.cm.register_cmap(None, cm)
        plt.set_cmap(cm)
        colors = [x[1] for x in trajectory_steps]
        scatter = plt.scatter(
            embedding[:, 0], embedding[:, 1], c=colors, cmap=cm, s=5, alpha=0.5
        )
        handles, _ = scatter.legend_elements()
        plt.legend(
            handles,
            [
                str(action_type).lstrip("ActionType.")
                for action_type in action_conversion.keys()
            ],
            loc="best",
            title="ActionTypes",
        )
...
def cluster_combined_trajectories(
    game_plays, filename=None, end_reason=None, optimal_gamelays=None
):
    action_conversion = {
        ActionType.ScanNetwork: 0,
        ActionType.FindServices: 1,
    ...
    reducer = umap.UMAP(
        metric="cosine", n_neighbors=50, min_dist=0.9, transform_seed=42
    ).fit(scaled_data)
    ...
    im1 = ax1.scatter(
        embedding[:, 0],
        embedding[:, 1],
        c=[x["timestamp"] for x in trajectory_steps],
        cmap="plasma",
        s=0.2,
        alpha=0.5,
    )
    ...
    scatter2 = ax2.scatter(
        embedding[:, 0],
        embedding[:, 1],
        c=[model_type_conversion[x["model"]] for x in trajectory_steps],
        s=0.2,
        cmap=cm,
        alpha=0.5,
    )
    ...
    try:
        ...
        ax2.legend(
            handles2,
            [model_type for model_type in model_type_conversion.keys()],
            loc="best",
        )
    except ValueError:
        pass
    ...
    cm = from_list(
        "Set4",
        plt.cm.Set1(range(0, len(end_reason_conversion))),
        len(end_reason_conversion),
    )
    cm = update_cmap(end_reason_conversion, plt.cm.Set1, "Set4")
    scatter3 = ax3.scatter(
        embedding[:, 0],
        embedding[:, 1],
        c=[end_reason_conversion[x["end_reason"]] for x in trajectory_steps],
        cmap=cm,
        s=0.2,
        alpha=0.5,
    )
    ...
    try:
        ...
        ax3.legend(
            handles3,
            [end_reason for end_reason in end_reason_conversion.keys()],
            loc="best",
        )
    except ValueError:
        pass
    ...
    scatter4 = ax4.scatter(
        embedding[:, 0],
        embedding[:, 1],
        c=[x["action_type"] for x in trajectory_steps],
        cmap=cm,
        s=0.2,
        alpha=0.5,
    )
...
def generate_mdp_from_trajecotries(
    game_plays: list, filename: str, end_reason=None
) -> dict:
    idx_mapping = {
        "Start": 0,
    ...
    for i in range(len(idx_mapping)):
        for j in range(len(idx_mapping)):
            _ = ax.text(j, i, transitions[i, j], ha="center", va="center", color="w")

    ax.set_title(f"Visualization of MDP for {play['model']}")
    fig.tight_layout()
    fig.savefig(
        os.path.join("figures", f"{filename}_{END_REASON if end_reason else ''}.png"),
        dpi=600,
    )
...
def gameplay_graph(game_plays: list, states, actions, end_reason=None) -> tuple:
    ...
    for play in game_plays:
        ...
        for i in range(1, len(play["trajectory"]["actions"])):
            state = utils.state_as_ordered_string(
                GameState.from_dict(play["trajectory"]["states"][i - 1])
            )
            next_state = utils.state_as_ordered_string(
                GameState.from_dict(play["trajectory"]["states"][i - 1])
            )
...
def get_graph_stats(edge_list, states, actions) -> tuple:
    ...
    print(
        f"Node in-degree - min:{np.min(list(incoming_edges.values()))}, max:{np.max(list(incoming_edges.values()))}, mean:{np.mean(list(incoming_edges.values()))}, std:{np.std(list(incoming_edges.values()))}"
    )
    print(
        f"Node out-degree - min:{np.min(list(outgoing_edges.values()))}, max:{np.max(list(outgoing_edges.values()))}, mean:{np.mean(list(outgoing_edges.values()))}, std:{np.std(list(outgoing_edges.values()))}"
    )
    print(
        f"Loops - min:{np.min(list(loops.values()))}, max:{np.max(list(loops.values()))}, mean:{np.mean(list(loops.values()))}, std:{np.std(list(loops.values()))}"
    )
...
if __name__ == "__main__":
    ...
    game_plays = read_json("./trajectories.jsonl")
    for play in game_plays:
        play["model"] = "Optimal"
