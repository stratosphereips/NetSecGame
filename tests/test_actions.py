...
class TestActionsNoDefender:
    ...
    def test_exploit_service_witout_find_service(self, env_obs_scan):
        ...
        obs = env.step(action)
        assert obs.state == observation.state
        assert components.Service('postgresql', 'passive', '14.3.0', False) not in obs.state.known_services[components.IP('192.168.1.3')]
