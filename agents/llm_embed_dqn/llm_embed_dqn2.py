...
class LLMEmbedAgent:
    ...
    def _create_model(self, embedding_size=384):
        ...
        inputs = tf.keras.layers.Input(shape=(embedding_size*2,))
        ...
        q_value = tf.keras.layers.Dense(len(self.all_actions))(layer3)
    ...
    def _select_best_action_for_state(self, state, memories):
        """
        ...
        #     try:
        q_value = self.q_model(tf.concat([state_embed.reshape(1, -1), memory_embed.reshape(1, -1)], axis=1))[0]
        action_id = np.argmax(q_value)
        print(q_value, action_id)
        print(self.all_actions[action_id])
    ...
    def _optimize_models(self):
        ...
        for _ in range(self.args.num_epochs):
            ...
            next_action_batch = np.array(batch.next_action)
            ...
            future_rewards = tf.math.reduce_max(self.q_target(tf.concat([next_state_batch, next_memory_batch], axis=1)), axis=1)
            # print("future:", future_rewards)
            updated_q_values = reward_batch + self.gamma * future_rewards * (1 - dones)

            # Create a mask so we only calculate loss on the updated Q-values
            masks = tf.one_hot(action_batch, len(self.all_actions))
            ...
            with tf.GradientTape() as tape:
                ...
                loss = self.q_model.loss(updated_q_values, q_action)
    ...
    def train(self, num_episodes): # pylint: disable=too-many-locals,too-many-statements
        ...
        for episode in range(1, num_episodes+1):
            ...
            action_id = self._select_best_action_for_state(observation.state, memories)
            ...
            for _ in range(self.args.max_t):
                ...
                action_next_id = self._select_best_action_for_state(observation_next.state, memories)
